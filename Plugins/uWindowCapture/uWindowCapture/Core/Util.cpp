...
bool IsUWP(DWORD pid)
{
    ...
    const auto GetPackageFamilyName =
        reinterpret_cast<GetPackageFamilyNameType>(
            ::GetProcAddress(hKernel32, "GetPackageFamilyName"));
    if (!GetPackageFamilyName) return false;

    // FIX #10: PROCESS_QUERY_LIMITED_INFORMATION for UWP protection support
...
DWORD GetStoreAppProcessId(HWND hWnd)
{
    ...
    static const auto EnumChildWindowsCallback =
        static_cast<EnumChildWindowsCallbackType>(_EnumChildWindowsCallback);
