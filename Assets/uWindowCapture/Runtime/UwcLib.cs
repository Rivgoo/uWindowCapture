...
namespace uWindowCapture
{
	...
	public static class Lib
	{
		...
		public static extern void UwcFinalize();
		...
		public static bool GetWindowPixels(int id, Color32[] colors, int x, int y, int width, int height)
		{
			...
			{
				Debug.LogErrorFormat("uWC: GetWindowPixels colors array is smaller than (width * height). id:{0} x:{1} y:{2} w:{3} h:{4}", id, x, y, width, height);
				return false;
			}
			...
			{
				Debug.LogErrorFormat("uWC: GetWindowPixels({0}, {1}, {2}, {3}, {4}) failed.", id, x, y, width, height);
				handle.Free();
				return false;
			}
			handle.Free();
